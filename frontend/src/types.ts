...
export type WeatherDefinition = { id: string; name: string; description: string; imagegen_description: string; tags: string[]; image_tags: string[]; enabled: boolean; transition_ids: string[]; visibility_multiplier?: number };
export type TimePhase = { id: string; name: string; duration_minutes: number; description: string; imagegen_description: string; position: number; enabled: boolean; visibility_multiplier?: number };
...
export type EnvironmentLocation = { id?: string; name: string; parent_location_id: string | null; exposure: "indoor" | "outdoor" | "isolated"; description: string; imagegen_description: string; tags: string[]; image_tags: string[]; enabled: boolean; random_encounter: boolean; hidden: boolean; discovered: boolean; x: number | null; y: number | null; topology: "open" | "closed"; occupancy: "direct_allowed" | "child_required"; boundary_access: "free" | "connection_required"; spatial_kind: "spot" | "area"; priority_layer: number; minutes_per_unit: number; base_visibility_units: number | null; encounter_rate: number; footprint?: Record<string, unknown> | null; local_bounds?: Record<string, unknown> | null };
export type EnvironmentSettings = { project_id: string; enabled: boolean; ai_create_locations: boolean; ai_propose_weather: boolean; auto_generate_backgrounds: boolean; background_workflow_id: string | null; initial_weather_id: string; perception_stat_key?: string | null; revision: number; weather: WeatherDefinition[]; time_phases: TimePhase[] };
...
export type LocationMapLayer = { root_location_id?: string | null; parent?: { id: string; name: string; parent_id?: string | null; topology?: "open" | "closed" } | null; breadcrumbs: Array<{ id: string; name: string }>; locations: Array<{ id: string; name: string; x: number; y: number; has_children: boolean; exposure: string; enabled: boolean; effectively_enabled: boolean; topology?: "open" | "closed"; occupancy?: "direct_allowed" | "child_required"; boundary_access?: "free" | "connection_required"; spatial_kind?: "spot" | "area"; discovered?: boolean }>; routes: Array<{ id: string; source_id: string; target_id: string; kind?: "route" | "door" | "portal"; locked?: boolean; hidden?: boolean }> };
...
export type RuleOwnerKind = "character" | "item" | "location" | "faction" | "lore_system" | "fact" | "plot_beat" | "relationship" | "ability" | "effect" | "weather" | "outfit" | "navigation_space" | "map_feature";
export type StatDefinition = { project_id?: string; stat_key: string; label: string; description: string; compatible_owner_kinds: RuleOwnerKind[]; default_value: number; minimum: number; maximum: number; integer_only: boolean; visibility: "public" | "private" | "narrator"; color?: string | null; minimum_stat_key?: string | null; maximum_stat_key?: string | null; minimum_color?: string | null; maximum_color?: string | null; icon?: string | null; display_style: "compact" | "bar" };
...
export type EffectDefinition = { project_id?: string; effect_key: string; name: string; description: string; target_stat_key: string; operation: "add" | "subtract" | "set" | "multiply"; formula: FormulaNode; value_expression?: Record<string, unknown> | null; clock: "story_minutes" | "target_actions" | "world_actions"; duration: number; tick_interval: number; evaluation_mode: "snapshot" | "live"; stacking_policy: "replace" | "refresh" | "stack" | "independent"; max_stacks: number; visibility: "public" | "private" | "narrator"; icon?: string | null; enabled: boolean; stats?: Record<string, number> };
...
export type RequirementExpression = {
  kind?: "and" | "or" | "not" | "compare" | "has_item" | "has_tag" | "relationship" | "location" | "time" | "weather" | "has_ability" | null;
  children?: RequirementExpression[];
  child?: RequirementExpression | null;
  target?: "actor" | "target";
