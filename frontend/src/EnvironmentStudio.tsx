...
const emptyLocation = (parentId: string | null): EnvironmentLocation => ({ name: "", parent_location_id: parentId, exposure: "outdoor", description: "", imagegen_description: "", tags: [], image_tags: [], enabled: true, random_encounter: false, discovered: true, x: null, y: null });
...
function normalizedLocation(entity: WorldEntity): EnvironmentLocation {
  ...
  return {
    ...
    random_encounter: Boolean(state.random_encounter), discovered: state.discovered == null ? !state.random_encounter : Boolean(state.discovered),
...
export function EnvironmentStudio({ projectId, revision, workflows, fail, focusLocationId, onFocusHandled }: { projectId: string; revision: number; workflows: WorkflowPreset[]; fail: (message: string) => void; focusLocationId?: string | null; onFocusHandled?: () => void }) {
  ...
  async function saveEditor() {
    ...
    try {
      ...
      if (editor.kind === "weather") {
        const body = { name: editor.value.name, description: editor.value.description, imagegen_description: editor.value.imagegen_description, tags: editor.value.tags, image_tags: editor.value.image_tags, enabled: editor.value.enabled };
  ...
  return <div className="page environment-page">
    ...
    <div className="environment-resource-grid">
      ...
      <ResourceList title="Locations" query={search.location} setQuery={value => setSearch({ ...search, location: value })} onAdd={() => begin({ kind: "location", value: emptyLocation(parentId) }, "location")}>{filterBy(locations.map(normalizedLocation), "location").map(item => <ResourceButton key={item.id} enabled={item.enabled} title={item.name} subtitle={item.exposure === "isolated" ? "Sealed" : item.exposure} onClick={() => begin({ kind: "location", value: item }, "location", item.id)} />)}</ResourceList>
    </div>
