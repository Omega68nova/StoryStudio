...
export function PlanningStudio({
  ...
  async function randomizeDirection() {
    ...
    try {
      const created = await api<{ id: string }>(`/projects/${projectId}/generation-plans/random-direction`, {
        method: "POST",
  ...
  async function saveStageDraft(stage: number, draft: any) {
    await api(`/generation-plans/planning/${session!.id}/tasks/${stage}/result`, {
      method: "PUT",
    ...
    try {
      ...
      if (approve && !suppliedResolutions) {
        const result = await api<{ conflicts: PlanningConflict[] }>(
          `/generation-plans/planning/${session!.id}/tasks/${stage}/preflight`,
      ...
      if (approve) {
        await api(`/generation-plans/planning/${session!.id}/tasks/${stage}/approve`, {
          method: "POST",
          body: JSON.stringify({
            draft,
