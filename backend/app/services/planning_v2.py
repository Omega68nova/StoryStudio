        ...
        5: {
            "characters": [{
                ...
                "state": {
                    ...
                    "goals": [],
                    "secrets": "General secret notes for the narrator.",
                    "character_secrets": [],
...
def resolve_resource(db: Database, session_id: str, key_or_id: str | None, expected_type: str | None = None) -> str | None:
    if not key_or_id:
        return None
    row = db.fetch_one("SELECT resource_id,resource_type FROM planning_resource_keys WHERE session_id=? AND resource_key=?", (session_id, key_or_id))
    if row and (not expected_type or row["resource_type"] == expected_type):
        return str(row["resource_id"])
    return str(key_or_id)


def record_resource(db: Database, session_id: str, stage_number: int, key: str, resource_type: str, resource_id: str, value: Any) -> None:
    db.execute(
        "INSERT INTO planning_resource_keys(session_id,resource_key,resource_type,resource_id,stage_number,fingerprint,updated_at) VALUES(?,?,?,?,?,?,?) "
        "ON CONFLICT(session_id,resource_key) DO UPDATE SET resource_type=excluded.resource_type,resource_id=excluded.resource_id,stage_number=excluded.stage_number,fingerprint=excluded.fingerprint,updated_at=excluded.updated_at",
        (session_id, key, resource_type, resource_id, stage_number, stable_hash(value), utc_now()),
    )
...
def apply_weather(db: Database, project_id: str, session_id: str, stage_number: int, draft: dict[str, Any]) -> None:
    now, ids = utc_now(), {}
    for weather in draft.get("weather", []):
        key = str(weather["key"]); existing = db.fetch_one("SELECT resource_id FROM planning_resource_keys WHERE session_id=? AND resource_key=? AND resource_type='weather'", (session_id, key))
        ...
        ids[key] = weather_id; record_resource(db, session_id, stage_number, key, "weather", weather_id, weather)
    ...
    for edge in draft.get("weather_transitions", []):
        source = ids.get(str(edge.get("source_key"))) or resolve_resource(db, session_id, edge.get("source_key"), "weather")
        target = ids.get(str(edge.get("target_key"))) or resolve_resource(db, session_id, edge.get("target_key"), "weather")
    ...
    initial = ids.get(str(draft.get("initial_weather_key"))) or resolve_resource(db, session_id, draft.get("initial_weather_key"), "weather")
    if initial:
...
def apply_rules(db: Database, project_id: str, session_id: str, stage_number: int, draft: dict[str, Any]) -> None:
    now = utc_now()
    for stat in draft.get("stats", []):
        ...
        existing = db.fetch_one("SELECT resource_id FROM planning_resource_keys WHERE session_id=? AND resource_key=? AND resource_type='stat'", (session_id, key))
        ...
        record_resource(db, session_id, stage_number, key, "stat", stat_id, stat)
    ...
    for ability in draft.get("abilities", []):
        ...
        existing = db.fetch_one("SELECT resource_id FROM planning_resource_keys WHERE session_id=? AND resource_key=? AND resource_type='ability'", (session_id, key)); ability_id = str(existing["resource_id"]) if existing else new_id()
        ...
        record_resource(db, session_id, stage_number, key, "ability", ability_id, ability)


def apply_outfits(db: Database, project_id: str, session_id: str, stage_number: int, draft: dict[str, Any]) -> None:
    now = utc_now()
    for outfit in draft.get("outfits", []):
        key = str(outfit.get("key") or "").strip(); entity_id = resolve_resource(db, session_id, outfit.get("character_key"), "entity")
        ...
        existing = db.fetch_one("SELECT resource_id FROM planning_resource_keys WHERE session_id=? AND resource_key=? AND resource_type='outfit'", (session_id, key)); outfit_id = str(existing["resource_id"]) if existing else new_id()
        ...
        record_resource(db, session_id, stage_number, key, "outfit", outfit_id, outfit)
    for routine in draft.get("routines", []):
        key = str(routine.get("key") or "").strip(); character_id = resolve_resource(db, session_id, routine.get("character_key"), "entity"); location_id = resolve_resource(db, session_id, routine.get("location_key"), "entity")
        ...
        existing = db.fetch_one("SELECT resource_id FROM planning_resource_keys WHERE session_id=? AND resource_key=? AND resource_type='routine'", (session_id, key)); routine_id = str(existing["resource_id"]) if existing else new_id()
        ...
        record_resource(db, session_id, stage_number, key, "routine", routine_id, routine)


def apply_runtime(db: Database, project_id: str, session_id: str, draft: dict[str, Any]) -> None:
    ...
    variants = {row["id"] for row in db.fetch_all("SELECT id FROM ambient_variants WHERE project_id=?", (project_id,))}
    for rule in draft.get("ambient", []):
        owner_type = str(rule.get("owner_type") or ""); owner_id = resolve_resource(db, session_id, rule.get("owner_key"))
...
def prepare_image_plans(db: Database, project_id: str, session_id: str, draft: dict[str, Any] | None = None) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        existing = db.fetch_one("SELECT id,status,prompt_revision,media_asset_id,generation_job_id FROM planning_image_plans WHERE session_id=? AND resource_key=?", (session_id, resource_key)); plan_id = existing["id"] if existing else new_id()
        ...
        else: db.execute("INSERT INTO planning_image_plans(id,session_id,project_id,resource_key,entity_id,kind,prompt,negative_prompt,workflow_preset_id,width,height,prompt_revision,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", (plan_id, session_id, project_id, resource_key, row["id"], kind, prompt, override.get("negative_prompt", ""), workflow_id, override.get("width"), override.get("height"), revision, next_status, now, now))
