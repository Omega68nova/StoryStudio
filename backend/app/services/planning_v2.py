...
def prepare_image_plans(db: Database, project_id: str, owner_id: str, draft: dict[str, Any] | None = None) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        existing = db.fetch_one("SELECT id,status,prompt_revision,media_asset_id,generation_job_id FROM generation_image_plans WHERE (generation_plan_id=? OR (generation_plan_id IS NULL AND session_id=?)) AND resource_key=? LIMIT 1", (generation_plan_id, legacy_session_id, resource_key)); plan_id = existing["id"] if existing else new_id()
