...
class SoundManager:
        ...
        if self.sound_root.is_dir():
            for path in self.sound_root.rglob("*"):
                ...
                row = self.db.fetch_one(
                    ...
                    "AND playback_rate=1 AND derived=0",
                    (
                        project_id,
                        relative,
                    ),
                ...
                if row:
                    self.db.execute(
                        "UPDATE ambient_variants "
                        "SET available=1,updated_at=? "
                        "WHERE id=?",
                        (
                            now,
                            row["id"],
                ...
                self.db.execute(
                    ...
                    "(id,project_id,source_path,label,"
    ...
    def resolve_ambient(
        ...
        *,
        ...
        action = str(
            projection.get("player_action")
            or "standing"
        ).casefold()

        assignments = self.db.fetch_all(
            "SELECT * FROM ambient_assignments "
            "WHERE project_id=?",
            (project_id,),
        )
        ...
        rows = self.db.fetch_all(
            ...
            "playback_rate,default_gain,tags_json "
            "FROM ambient_variants "
    ...
    def ambient_sets(
        ...
        rows = self.db.fetch_all(
            ...
            "weather_id,time_phase_id,variant_id "
